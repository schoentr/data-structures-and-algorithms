...
const count = (target, input) => {
  if(input ===[])return 0;
  let numX =input.reduce( (outAcc,cur) => {
    let num1 =cur.reduce( (acc1,curr) =>{
      if (curr === target){
        outAcc++;
      }
    }, 1);
...
const divisibleByFiveTwoToThePower = (input) => {
  let new2Arr=[];
  return input.reduce( (outAcc,cur) => {
    let newArr = cur.filter(num => num % 5 ===0 && !num.length>0);
    // console.log('newArr:',newArr);
    let mapArr=newArr.map( x =>Math.pow(2,x));
    new2Arr.push(mapArr);
    return new2Arr;}
  ,[]);
...
let findMaleAndFemale = (data) => {
  let maleFemale= data.reduce((acc,curr) =>{
