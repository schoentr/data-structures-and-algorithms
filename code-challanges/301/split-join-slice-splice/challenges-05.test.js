...
const listFoods = (recipe) => {
  let wordArr =[];
  recipe.ingredients.forEach(function(value,index,arr){
    let spaceArr = [];
    for (let i =0; i <value.length; i ++){
      if (value[i]===' ')spaceArr.push(i+1);
    }
    wordArr.push(value.slice(spaceArr[1], value.length));
  });
  return wordArr;
};
...
const splitFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(function(value,index,arr){
    ...
    wordsArr.splice(0,2);
    let sent='';
    wordsArr.forEach(function(value,index,arr1){
      sent += arr1[index];
      if (index < arr1.length-1) sent+=' ';
    });
    arrA.push(sent);
  });
  return arrA;
...
const stepActions = (recipe) => {
  let result = [];
  recipe.steps.forEach(function(value,index,arr){
    let spaceArr = [];
    for (let i =0; i <value.length; i ++){
      if (value[i]===' ')spaceArr.push(i+1);
    }
    result.push(value.slice(0, spaceArr[0]-1));
  });
  return result;
};
...
const removeEvenValues = (arr) => {
  ...
  for (let i =0 ; i < length; i ++){
    if (arr[i] % 2 === 0) {
      evenNumberIndex.push(i);
      console.log(i, arr);
    }
  }
  for(let i = evenNumberIndex.length; i >= 0; i--){
    arr.splice(i,i);
  }
};
/* ------------------------------------------------------------------------------------------------
...
const removeLastCharacters = (str, numberOfCharacters) =>{
  if (numberOfCharacters < 1) return str;
