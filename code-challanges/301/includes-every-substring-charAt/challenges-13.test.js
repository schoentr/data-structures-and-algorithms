...
const firstLetters = (arr) => {
  let newArr = arr.map( str => str.charAt(0));
  return newArr;
};
...
const findAnything = (arr, target) => {
  let newArr= [];
  arr.forEach(str =>{
    if (str.includes(target)) newArr.push(str);
  });
  return newArr;
...
const findEvery = (arr, target) => {
  ...
  let test = true;
  arr.forEach(str =>{
    if(!str.includes(target)) test = false;
  });
  return test;
  // Solution code here...
};
...
const unenrollBrook = (arr) => {
  ...
  arr.forEach(innerArr =>{
    ...
    innerArr.forEach(str=>{
      if (!str.includes('Brook')) newInnerArry.push(str);
    });
    newArr.push(newInnerArry);
  });
  return newArr;
